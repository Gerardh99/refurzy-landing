import { Vacature, Kandidaat, Sollicitatie, Contract } from './types'

export const vacatures: Vacature[] = [
  {
    id: 'vac-1',
    title: 'Marketing Manager',
    company: 'TechVentures B.V.',
    location: 'Amsterdam',
    land: 'Nederland',
    vakgebied: 'Marketing & E-commerce',
    salaris: '€4.500 - €6.000',
    deadline: '2026-04-15',
    status: 'open',
    createdAt: '2026-03-01',
    pmsCompleted: true,
    exclusiviteit: true,
    scoutVergoeding: 4500,
    hardeCriteria: {
      opleidingsniveau: 'HBO',
      minimaleErvaring: '5-10 jaar',
      locatie: 'Amsterdam',
      opKantoor: 'Hybride (3 dagen)',
      maxReistijd: '45 minuten',
      salarisMin: 4500,
      salarisMax: 6000,
      talen: [
        { taal: 'Nederlands', minimaalNiveau: 'C1' },
        { taal: 'Engels', minimaalNiveau: 'B2' },
      ],
    },
    salarisMin: 4500,
    salarisMax: 6000,
    kandidaten: [
      {
        id: 'k-1', naam: 'Anna de Jong', initialen: 'A', anoniem: false,
        hardeCriteriaMatch: 100, hardeCriteriaIcon: 'check',
        deVriesFit: 87, scoutRating: 4.8, scoutNaam: 'Sophie de Graaf', scoutId: 'scout-1',
        status: 'aanbevolen', procesStatus: 'arbeidsvoorwaarden', unlocked: true,
        opleidingsniveau: 'WO', werkervaring: '8 jaar', woonplaats: 'Amsterdam',
        email: 'anna.dejong@email.nl', telefoon: '06-12345678',
        gesprekken: [
          { id: 'g-1', type: 'kennismaking', datum: '2026-03-10', feedback: 'Zeer sterke kandidaat. Goede culturele fit, enthousiast over de rol. Inhoudelijk sterk.', rating: 5, status: 'afgerond' },
          { id: 'g-2', type: 'verdieping', datum: '2026-03-14', feedback: 'Team was ook enthousiast. Past goed bij de bedrijfscultuur.', rating: 5, status: 'afgerond' },
        ],
        stapStartDatum: '2026-03-17',
        laatsteActiviteit: '2026-03-19',
        gesprekDatum: '2026-03-10',
        kandidaatBevestigd: true,
        feedbackScore: 5,
        feedbackSamenvatting: 'Uitstekend — team unaniem positief',
        vervolggesprekDatum: '2026-03-14',
        arbeidsvoorwaardenStatus: 'in_onderhandeling',
      },
      {
        id: 'k-10', naam: 'Nadia Karim', initialen: 'N', anoniem: true,
        hardeCriteriaMatch: 88, hardeCriteriaIcon: 'check',
        deVriesFit: 73, scoutRating: 4.8, scoutNaam: 'Sophie de Graaf', scoutId: 'scout-1',
        status: 'bekijk', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'WO', werkervaring: '4 jaar', woonplaats: 'Leiden',
        stapStartDatum: '2026-03-18',
        laatsteActiviteit: '2026-03-18',
      },
      {
        id: 'k-3', naam: 'Lisa Brouwer', initialen: 'L', anoniem: true,
        hardeCriteriaMatch: 76, hardeCriteriaIcon: 'warning',
        deVriesFit: 53, scoutRating: 3.7, scoutNaam: 'Mark Jansen', scoutId: 'scout-2',
        status: 'overweeg', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'HBO', werkervaring: '4 jaar', woonplaats: 'Den Haag',
      },
      {
        id: 'k-20', naam: 'Mees van Leeuwen', initialen: 'M', anoniem: true,
        hardeCriteriaMatch: 83, hardeCriteriaIcon: 'check',
        deVriesFit: 79, scoutRating: 3.7, scoutNaam: 'Mark Jansen', scoutId: 'scout-2',
        status: 'bekijk', procesStatus: 'contract_akkoord', unlocked: true,
        opleidingsniveau: 'HBO', werkervaring: '6 jaar', woonplaats: 'Haarlem',
        email: 'mees.vanleeuwen@email.nl', telefoon: '06-11223344',
        stapStartDatum: '2026-03-16',
        laatsteActiviteit: '2026-03-17',
      },
      {
        id: 'k-21', naam: 'Yara Bosman', initialen: 'Y', anoniem: true,
        hardeCriteriaMatch: 67, hardeCriteriaIcon: 'warning',
        deVriesFit: 91, scoutRating: 4.2, scoutNaam: 'Eva Hendriks', scoutId: 'scout-3',
        status: 'bekijk', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'WO', werkervaring: '3 jaar', woonplaats: 'Utrecht',
        stapStartDatum: '2026-03-19',
        laatsteActiviteit: '2026-03-19',
      },
      {
        id: 'k-40', naam: 'Demi Hoekstra', initialen: 'D', anoniem: true,
        hardeCriteriaMatch: 83, hardeCriteriaIcon: 'check',
        deVriesFit: 74, scoutRating: 0, scoutNaam: 'Lotte van der Berg', scoutId: 'scout-4',
        eersteVoordracht: true,
        status: 'bekijk', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'HBO', werkervaring: '6 jaar', woonplaats: 'Almere',
        stapStartDatum: '2026-03-20',
        laatsteActiviteit: '2026-03-20',
      },
      {
        id: 'k-22', naam: 'Bram Smeets', initialen: 'B', anoniem: false,
        hardeCriteriaMatch: 100, hardeCriteriaIcon: 'check',
        deVriesFit: 65, scoutRating: 4.2, scoutNaam: 'Eva Hendriks', scoutId: 'scout-3',
        status: 'overweeg', procesStatus: 'gesprek_gepland', unlocked: true,
        opleidingsniveau: 'WO', werkervaring: '9 jaar', woonplaats: 'Amsterdam',
        email: 'bram.smeets@email.nl', telefoon: '06-22334455',
        gesprekken: [
          { id: 'g-10', type: 'kennismaking', datum: '2026-03-22', status: 'gepland' },
        ],
        stapStartDatum: '2026-03-14',
        laatsteActiviteit: '2026-03-18',
        gesprekDatum: '2026-03-22',
        kandidaatBevestigd: true,
      },
    ],
  },
  {
    id: 'vac-2',
    title: 'Senior Software Developer',
    company: 'TechVentures B.V.',
    location: 'Amsterdam / Remote',
    land: 'Nederland',
    vakgebied: 'ICT & Development',
    salaris: '€5.500 - €7.500',
    deadline: '2026-04-30',
    status: 'open',
    createdAt: '2026-03-10',
    pmsCompleted: true,
    exclusiviteit: false,
    scoutVergoeding: 3600,
    hardeCriteria: {
      opleidingsniveau: 'HBO',
      minimaleErvaring: '5-10 jaar',
      locatie: 'Amsterdam / Remote',
      opKantoor: 'Hybride (2 dagen)',
      maxReistijd: '60 minuten',
      salarisMin: 5500,
      salarisMax: 7500,
      talen: [
        { taal: 'Engels', minimaalNiveau: 'C1' },
      ],
    },
    salarisMin: 5500,
    salarisMax: 7500,
    kandidaten: [
      {
        id: 'k-4', naam: 'Thomas van Dijk', initialen: 'T', anoniem: false,
        hardeCriteriaMatch: 95, hardeCriteriaIcon: 'check',
        deVriesFit: 82, scoutRating: 4.8, scoutNaam: 'Sophie de Graaf', scoutId: 'scout-1',
        status: 'aanbevolen', procesStatus: 'gesprek_plannen', unlocked: true,
        opleidingsniveau: 'WO', werkervaring: '10 jaar', woonplaats: 'Amsterdam',
        email: 'thomas.vdijk@email.nl', telefoon: '06-34567890',
        gesprekken: [],
        stapStartDatum: '2026-03-15',
        laatsteActiviteit: '2026-03-15',
      },
      {
        id: 'k-5', naam: 'Priya Sharma', initialen: 'P', anoniem: true,
        hardeCriteriaMatch: 88, hardeCriteriaIcon: 'check',
        deVriesFit: 68, scoutRating: 3.7, scoutNaam: 'Mark Jansen', scoutId: 'scout-2',
        status: 'bekijk', procesStatus: 'feedback_geven', unlocked: true,
        opleidingsniveau: 'HBO', werkervaring: '7 jaar', woonplaats: 'Rotterdam',
        email: 'priya.sharma@email.nl', telefoon: '06-45678901',
        gesprekken: [
          { id: 'g-3', type: 'kennismaking', datum: '2026-03-12', status: 'afgerond' },
        ],
      },
      {
        id: 'k-23', naam: 'Milan Kuipers', initialen: 'M', anoniem: true,
        hardeCriteriaMatch: 100, hardeCriteriaIcon: 'check',
        deVriesFit: 76, scoutRating: 4.8, scoutNaam: 'Sophie de Graaf', scoutId: 'scout-1',
        status: 'aanbevolen', procesStatus: 'gesprek_gepland', unlocked: true,
        opleidingsniveau: 'WO', werkervaring: '8 jaar', woonplaats: 'Utrecht',
        email: 'milan.kuipers@email.nl', telefoon: '06-33445566',
        gesprekken: [
          { id: 'g-11', type: 'kennismaking', datum: '2026-03-24', status: 'gepland' },
        ],
        stapStartDatum: '2026-03-16',
        laatsteActiviteit: '2026-03-18',
        gesprekDatum: '2026-03-24',
        kandidaatBevestigd: false,
      },
      {
        id: 'k-24', naam: 'Fleur Willems', initialen: 'F', anoniem: true,
        hardeCriteriaMatch: 83, hardeCriteriaIcon: 'check',
        deVriesFit: 62, scoutRating: 3.7, scoutNaam: 'Mark Jansen', scoutId: 'scout-2',
        status: 'overweeg', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'HBO', werkervaring: '6 jaar', woonplaats: 'Den Haag',
        stapStartDatum: '2026-03-19',
        laatsteActiviteit: '2026-03-19',
      },
      {
        id: 'k-41', naam: 'Stijn Groen', initialen: 'S', anoniem: true,
        hardeCriteriaMatch: 88, hardeCriteriaIcon: 'check',
        deVriesFit: 71, scoutRating: 0, scoutNaam: 'Lotte van der Berg', scoutId: 'scout-4',
        eersteVoordracht: true,
        status: 'bekijk', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'WO', werkervaring: '5 jaar', woonplaats: 'Amersfoort',
        stapStartDatum: '2026-03-21',
        laatsteActiviteit: '2026-03-21',
      },
      {
        id: 'k-25', naam: 'Jesse Mulder', initialen: 'J', anoniem: false,
        hardeCriteriaMatch: 72, hardeCriteriaIcon: 'warning',
        deVriesFit: 88, scoutRating: 4.2, scoutNaam: 'Eva Hendriks', scoutId: 'scout-3',
        status: 'bekijk', procesStatus: 'vervolggesprek', unlocked: true,
        opleidingsniveau: 'WO', werkervaring: '12 jaar', woonplaats: 'Amsterdam',
        email: 'jesse.mulder@email.nl', telefoon: '06-44556677',
        gesprekken: [
          { id: 'g-12', type: 'kennismaking', datum: '2026-03-11', feedback: 'Technisch zeer sterk. Ervaring overtreft verwachtingen.', rating: 5, status: 'afgerond' },
          { id: 'g-13', type: 'verdieping', datum: '2026-03-20', status: 'gepland' },
        ],
        stapStartDatum: '2026-03-18',
        laatsteActiviteit: '2026-03-20',
        feedbackScore: 5,
        feedbackSamenvatting: 'Top kandidaat — technisch ongeëvenaard',
        vervolggesprekDatum: '2026-03-20',
      },
    ],
  },
]

export const allVacatures: Vacature[] = [
  ...vacatures,
  {
    id: 'vac-3',
    title: 'HR Business Partner',
    company: 'GreenLogistics B.V.',
    location: 'Rotterdam',
    land: 'Nederland',
    vakgebied: 'HR & Recruitment',
    salaris: '€4.000 - €5.500',
    deadline: '2026-04-20',
    status: 'open',
    createdAt: '2026-03-05',
    pmsCompleted: true,
    exclusiviteit: false,
    scoutVergoeding: 2400,
    hardeCriteria: {
      opleidingsniveau: 'HBO',
      minimaleErvaring: '2-5 jaar',
      locatie: 'Rotterdam',
      opKantoor: 'Op kantoor (5 dagen)',
      maxReistijd: '30 minuten',
      salarisMin: 4000,
      salarisMax: 5500,
      talen: [
        { taal: 'Nederlands', minimaalNiveau: 'C1' },
        { taal: 'Engels', minimaalNiveau: 'B1' },
      ],
    },
    salarisMin: 4000,
    salarisMax: 5500,
    kandidaten: [
      {
        id: 'k-30', naam: 'Iris Dekker', initialen: 'I', anoniem: true,
        hardeCriteriaMatch: 100, hardeCriteriaIcon: 'check',
        deVriesFit: 84, scoutRating: 4.8, scoutNaam: 'Sophie de Graaf', scoutId: 'scout-1',
        status: 'aanbevolen', procesStatus: 'gesprek_plannen', unlocked: true,
        opleidingsniveau: 'WO', werkervaring: '4 jaar', woonplaats: 'Rotterdam',
        email: 'iris.dekker@email.nl', telefoon: '06-55667788',
        stapStartDatum: '2026-03-17',
        laatsteActiviteit: '2026-03-18',
      },
      {
        id: 'k-31', naam: 'Ruben Kok', initialen: 'R', anoniem: true,
        hardeCriteriaMatch: 83, hardeCriteriaIcon: 'check',
        deVriesFit: 71, scoutRating: 3.7, scoutNaam: 'Mark Jansen', scoutId: 'scout-2',
        status: 'bekijk', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'HBO', werkervaring: '3 jaar', woonplaats: 'Dordrecht',
        stapStartDatum: '2026-03-19',
        laatsteActiviteit: '2026-03-19',
      },
    ],
  },
  {
    id: 'vac-4',
    title: 'Financial Controller',
    company: 'MedTech Solutions',
    location: 'Eindhoven',
    land: 'Nederland',
    vakgebied: 'Accountancy & Finance',
    salaris: '€5.000 - €6.500',
    deadline: '2026-05-01',
    status: 'open',
    createdAt: '2026-03-12',
    pmsCompleted: true,
    exclusiviteit: true,
    scoutVergoeding: 6750,
    hardeCriteria: {
      opleidingsniveau: 'WO',
      minimaleErvaring: '5-10 jaar',
      locatie: 'Eindhoven',
      opKantoor: 'Hybride (3 dagen)',
      maxReistijd: '45 minuten',
      salarisMin: 5000,
      salarisMax: 6500,
      talen: [
        { taal: 'Nederlands', minimaalNiveau: 'C1' },
        { taal: 'Engels', minimaalNiveau: 'B2' },
      ],
    },
    salarisMin: 5000,
    salarisMax: 6500,
    kandidaten: [
      {
        id: 'k-32', naam: 'Sander de Vries', initialen: 'S', anoniem: true,
        hardeCriteriaMatch: 100, hardeCriteriaIcon: 'check',
        deVriesFit: 78, scoutRating: 4.8, scoutNaam: 'Sophie de Graaf', scoutId: 'scout-1',
        status: 'aanbevolen', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'WO', werkervaring: '7 jaar', woonplaats: 'Eindhoven',
        stapStartDatum: '2026-03-18',
        laatsteActiviteit: '2026-03-18',
      },
      {
        id: 'k-33', naam: 'Lotte Janssen', initialen: 'L', anoniem: true,
        hardeCriteriaMatch: 67, hardeCriteriaIcon: 'warning',
        deVriesFit: 82, scoutRating: 3.7, scoutNaam: 'Mark Jansen', scoutId: 'scout-2',
        status: 'bekijk', procesStatus: 'voorgesteld', unlocked: false,
        opleidingsniveau: 'HBO', werkervaring: '5 jaar', woonplaats: 'Tilburg',
        stapStartDatum: '2026-03-20',
        laatsteActiviteit: '2026-03-20',
      },
    ],
  },
]

// ─── Archived (completed) vacatures ──────────────────────────────────────────
export const archiefVacatures: Vacature[] = [
  {
    id: 'vac-arch-1',
    title: 'Product Owner',
    company: 'TechVentures B.V.',
    location: 'Amsterdam',
    land: 'Nederland',
    vakgebied: 'ICT & Development',
    salaris: '€4.500 - €6.000',
    deadline: '2025-09-15',
    status: 'vervuld',
    createdAt: '2025-06-01',
    pmsCompleted: true,
    hardeCriteria: {
      opleidingsniveau: 'HBO',
      minimaleErvaring: '5-10 jaar',
      locatie: 'Amsterdam',
      opKantoor: 'Hybride (3 dagen)',
      maxReistijd: '45 minuten',
      salarisMin: 4500,
      salarisMax: 6000,
      talen: [
        { taal: 'Nederlands', minimaalNiveau: 'C1' },
        { taal: 'Engels', minimaalNiveau: 'B2' },
      ],
    },
    salarisMin: 4500,
    salarisMax: 6000,
    kandidaten: [
      {
        id: 'k-arch-1', naam: 'Sanne Visser', initialen: 'S', anoniem: false,
        hardeCriteriaMatch: 100, hardeCriteriaIcon: 'check',
        deVriesFit: 74, scoutRating: 3.7, scoutNaam: 'Mark Jansen', scoutId: 'scout-2',
        status: 'aangenomen', procesStatus: 'contract_getekend', unlocked: true,
        opleidingsniveau: 'HBO', werkervaring: '7 jaar', woonplaats: 'Amsterdam',
        email: 'sanne.visser@email.nl', telefoon: '06-55566677',
        contractDatum: '2025-08-20',
      },
    ],
  },
  {
    id: 'vac-arch-2',
    title: 'Digital Marketing Lead',
    company: 'TechVentures B.V.',
    location: 'Amsterdam',
    land: 'Nederland',
    vakgebied: 'Marketing & E-commerce',
    salaris: '€4.000 - €5.500',
    deadline: '2025-12-01',
    status: 'vervuld',
    createdAt: '2025-09-15',
    pmsCompleted: true,
    hardeCriteria: {
      opleidingsniveau: 'HBO',
      minimaleErvaring: '2-5 jaar',
      locatie: 'Amsterdam',
      opKantoor: 'Hybride (2 dagen)',
      maxReistijd: '60 minuten',
      salarisMin: 4000,
      salarisMax: 5500,
      talen: [
        { taal: 'Nederlands', minimaalNiveau: 'B2' },
        { taal: 'Engels', minimaalNiveau: 'B2' },
      ],
    },
    salarisMin: 4000,
    salarisMax: 5500,
    kandidaten: [
      {
        id: 'k-arch-2', naam: 'Jamal Usan', initialen: 'J', anoniem: false,
        hardeCriteriaMatch: 92, hardeCriteriaIcon: 'check',
        deVriesFit: 71, scoutRating: 4.8, scoutNaam: 'Sophie de Graaf', scoutId: 'scout-1',
        status: 'aangenomen', procesStatus: 'contract_getekend', unlocked: true,
        opleidingsniveau: 'HBO', werkervaring: '6 jaar', woonplaats: 'Utrecht',
        email: 'jamal.usan@email.nl', telefoon: '06-23456789',
        contractDatum: '2025-11-10',
      },
    ],
  },
]

export const scoutKandidaten: Kandidaat[] = [
  // === BESCHIKBAAR ===
  {
    id: 'k-6', naam: 'Eva Bakker', email: 'eva.bakker@email.nl',
    telefoon: '06-45678901', woonplaats: 'Haarlem',
    opleidingsniveau: 'HBO', werkervaring: '3 jaar',
    huidigeRol: 'Junior Accountant',
    voorkeursFunctiegebied1: 'Accountancy & Finance', voorkeursFunctietitel1: 'Financial Controller',
    voorkeursFunctiegebied2: 'Risk & Compliance', voorkeursFunctietitel2: 'Risk Analyst',
    salarisMin: 3000, salarisMax: 4000, maxReistijd: '30 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }],
    scanCompleted: false, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-7', naam: 'Sophie Vermeer', email: 'sophie.vermeer@email.nl',
    telefoon: '06-56789012', woonplaats: 'Den Haag',
    opleidingsniveau: 'WO', werkervaring: '5 jaar',
    huidigeRol: 'HR Business Partner bij MediaGroup',
    voorkeursFunctiegebied1: 'HR & Recruitment', voorkeursFunctietitel1: 'HR Manager',
    voorkeursFunctiegebied2: 'Management & Directie', voorkeursFunctietitel2: 'People Director',
    salarisMin: 4500, salarisMax: 5500, maxReistijd: '45 minuten', opKantoor: 'Hybride (2 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'C1' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1', 'scout-2'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-8', naam: 'Daan Hendriks', email: 'daan.hendriks@email.nl',
    telefoon: '06-67890123', woonplaats: 'Rotterdam',
    opleidingsniveau: 'HBO', werkervaring: '4 jaar',
    huidigeRol: 'Frontend Developer bij StartupX',
    voorkeursFunctiegebied1: 'ICT & Development', voorkeursFunctietitel1: 'Full Stack Developer',
    voorkeursFunctiegebied2: 'Design & Creative', voorkeursFunctietitel2: 'UX Engineer',
    salarisMin: 3500, salarisMax: 4500, maxReistijd: '45 minuten', opKantoor: 'Hybride (2 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C1' }, { taal: 'Engels', niveau: 'C1' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1', 'scout-3'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-9', naam: 'Fatima El Amrani', email: 'fatima.elamrani@email.nl',
    telefoon: '06-78901234', woonplaats: 'Amsterdam',
    opleidingsniveau: 'WO', werkervaring: '7 jaar',
    huidigeRol: 'Data Analyst bij ConsultCo',
    voorkeursFunctiegebied1: 'Data & Analytics', voorkeursFunctietitel1: 'Data Science Lead',
    voorkeursFunctiegebied2: 'ICT & Development', voorkeursFunctietitel2: 'Machine Learning Engineer',
    salarisMin: 5000, salarisMax: 6500, maxReistijd: '30 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'C1' }, { taal: 'Arabisch', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1', 'scout-2'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-10', naam: 'Lars de Boer', email: 'lars.deboer@email.nl',
    telefoon: '06-89012345', woonplaats: 'Eindhoven',
    opleidingsniveau: 'HBO', werkervaring: '2 jaar',
    huidigeRol: 'Junior Sales Executive bij TechSales',
    voorkeursFunctiegebied1: 'Sales & Business Development', voorkeursFunctietitel1: 'Account Manager',
    voorkeursFunctiegebied2: 'Marketing & E-commerce', voorkeursFunctietitel2: 'Business Development Manager',
    salarisMin: 2800, salarisMax: 3500, maxReistijd: '45 minuten', opKantoor: 'Op kantoor (5 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: false, scoutIds: ['scout-1'],
    poolStatus: 'beschikbaar',
  },
  // === GEPLAATST ===
  {
    id: 'k-1', naam: 'Anna de Jong', email: 'anna.dejong@email.nl',
    telefoon: '06-12345678', woonplaats: 'Amsterdam',
    opleidingsniveau: 'WO', werkervaring: '8 jaar',
    huidigeRol: 'Marketing Manager bij TechVentures B.V.',
    voorkeursFunctiegebied1: 'Marketing & E-commerce', voorkeursFunctietitel1: 'Marketing Manager',
    voorkeursFunctiegebied2: 'Management & Directie', voorkeursFunctietitel2: 'Head of Marketing',
    salarisMin: 4500, salarisMax: 6000, maxReistijd: '45 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'geplaatst',
    plaatsing: { vacatureTitle: 'Marketing Manager', bedrijf: 'TechVentures B.V.', datum: '2026-02-15', fee: 10800, scoutFee: 5400 },
  },
  {
    id: 'k-2', naam: 'Jamal Usan', email: 'jamal.usan@email.nl',
    telefoon: '06-23456789', woonplaats: 'Utrecht',
    opleidingsniveau: 'HBO', werkervaring: '6 jaar',
    huidigeRol: 'Marketing Specialist bij BrandHouse',
    voorkeursFunctiegebied1: 'Marketing & E-commerce', voorkeursFunctietitel1: 'Marketing Manager',
    voorkeursFunctiegebied2: 'Communicatie & PR', voorkeursFunctietitel2: 'Brand Strategist',
    salarisMin: 3500, salarisMax: 4500, maxReistijd: '60 minuten', opKantoor: 'Hybride (2 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C1' }, { taal: 'Engels', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'geplaatst',
    plaatsing: { vacatureTitle: 'Digital Marketing Lead', bedrijf: 'BrandHouse B.V.', datum: '2026-01-20', fee: 7200, scoutFee: 3600 },
  },
  {
    id: 'k-4', naam: 'Thomas van Dijk', email: 'thomas.vdijk@email.nl',
    telefoon: '06-34567890', woonplaats: 'Amsterdam',
    opleidingsniveau: 'WO', werkervaring: '10 jaar',
    huidigeRol: 'Senior Developer bij FinTechCo',
    voorkeursFunctiegebied1: 'ICT & Development', voorkeursFunctietitel1: 'Senior Software Developer',
    voorkeursFunctiegebied2: 'Management & Directie', voorkeursFunctietitel2: 'Engineering Manager',
    salarisMin: 5500, salarisMax: 7500, maxReistijd: '30 minuten', opKantoor: 'Hybride (2 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'C1' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'geplaatst',
    plaatsing: { vacatureTitle: 'Lead Software Engineer', bedrijf: 'FinTechCo', datum: '2025-11-10', fee: 12000, scoutFee: 6000 },
  },
  // === INACTIEF ===
  {
    id: 'k-11', naam: 'Roos van der Berg', email: 'roos.vdberg@email.nl',
    telefoon: '06-90123456', woonplaats: 'Groningen',
    opleidingsniveau: 'WO', werkervaring: '6 jaar',
    huidigeRol: 'UX Designer bij DesignStudio',
    voorkeursFunctiegebied1: 'Design & Creative', voorkeursFunctietitel1: 'Senior UX Designer',
    voorkeursFunctiegebied2: 'ICT & Development', voorkeursFunctietitel2: 'Product Designer',
    salarisMin: 4500, salarisMax: 5500, maxReistijd: '60 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'inactief',
    inactiefReden: 'Zelf teruggetrokken — ander aanbod geaccepteerd',
    inactiefDatum: '2026-03-05',
  },
  {
    id: 'k-12', naam: 'Pieter Jansen', email: 'pieter.jansen@email.nl',
    telefoon: '06-01234567', woonplaats: 'Leiden',
    opleidingsniveau: 'HBO', werkervaring: '9 jaar',
    huidigeRol: 'Operations Manager bij LogiCorp',
    voorkeursFunctiegebied1: 'Operations', voorkeursFunctietitel1: 'Operations Director',
    voorkeursFunctiegebied2: 'Logistiek & Transport', voorkeursFunctietitel2: 'Supply Chain Director',
    salarisMin: 5000, salarisMax: 6500, maxReistijd: '45 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }, { taal: 'Duits', niveau: 'B1' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'inactief',
    inactiefReden: 'Tijdelijk niet beschikbaar — persoonlijke redenen',
    inactiefDatum: '2026-02-28',
  },
  // === EXTRA BESCHIKBAAR ===
  {
    id: 'k-40', naam: 'Nina Groen', email: 'nina.groen@email.nl',
    telefoon: '06-10203040', woonplaats: 'Breda',
    opleidingsniveau: 'HBO', werkervaring: '5 jaar',
    huidigeRol: 'Content Manager bij MediaWorks',
    voorkeursFunctiegebied1: 'Marketing & E-commerce', voorkeursFunctietitel1: 'Marketing Manager',
    voorkeursFunctiegebied2: 'Communicatie & PR', voorkeursFunctietitel2: 'Corporate Communicatieadviseur',
    salarisMin: 3500, salarisMax: 4500, maxReistijd: '45 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-41', naam: 'Tim Scholten', email: 'tim.scholten@email.nl',
    telefoon: '06-20304050', woonplaats: 'Arnhem',
    opleidingsniveau: 'WO', werkervaring: '3 jaar',
    huidigeRol: 'Junior Controller bij AccountPlus',
    voorkeursFunctiegebied1: 'Accountancy & Finance', voorkeursFunctietitel1: 'Financial Controller',
    voorkeursFunctiegebied2: 'Accountancy & Finance', voorkeursFunctietitel2: 'Business Controller',
    salarisMin: 3500, salarisMax: 5000, maxReistijd: '45 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1', 'scout-2'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-42', naam: 'Lieke Maas', email: 'lieke.maas@email.nl',
    telefoon: '06-30405060', woonplaats: 'Den Bosch',
    opleidingsniveau: 'HBO', werkervaring: '8 jaar',
    huidigeRol: 'Projectmanager bij BuildIt B.V.',
    voorkeursFunctiegebied1: 'Bouw & Civiele Techniek', voorkeursFunctietitel1: 'Project Manager',
    voorkeursFunctiegebied2: 'Operations', voorkeursFunctietitel2: 'Program Manager',
    salarisMin: 4000, salarisMax: 5500, maxReistijd: '60 minuten', opKantoor: 'Hybride (2 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B2' }, { taal: 'Duits', niveau: 'B1' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-43', naam: 'Stijn Peters', email: 'stijn.peters@email.nl',
    telefoon: '06-40506070', woonplaats: 'Maastricht',
    opleidingsniveau: 'WO', werkervaring: '11 jaar',
    huidigeRol: 'Head of Sales bij SalesForce NL',
    voorkeursFunctiegebied1: 'Sales & Business Development', voorkeursFunctietitel1: 'Sales Director',
    voorkeursFunctiegebied2: 'Management & Directie', voorkeursFunctietitel2: 'Commercial Director',
    salarisMin: 6000, salarisMax: 8000, maxReistijd: '60 minuten', opKantoor: 'Hybride (3 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'C1' }, { taal: 'Duits', niveau: 'B2' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1', 'scout-3'],
    poolStatus: 'beschikbaar',
  },
  {
    id: 'k-44', naam: 'Anouk Vos', email: 'anouk.vos@email.nl',
    telefoon: '06-50607080', woonplaats: 'Almere',
    opleidingsniveau: 'MBO', werkervaring: '4 jaar',
    huidigeRol: 'Administratief Medewerker bij AdminPro',
    voorkeursFunctiegebied1: 'Administratie', voorkeursFunctietitel1: 'Office Manager',
    voorkeursFunctiegebied2: 'Facilitair & Services', voorkeursFunctietitel2: 'Facility Coördinator',
    salarisMin: 2500, salarisMax: 3200, maxReistijd: '30 minuten', opKantoor: 'Op kantoor (5 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C2' }, { taal: 'Engels', niveau: 'B1' }],
    scanCompleted: false, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'beschikbaar',
  },
  // === EXTRA IN PROCES ===
  {
    id: 'k-45', naam: 'Hugo van Dam', email: 'hugo.vandam@email.nl',
    telefoon: '06-60708090', woonplaats: 'Leiden',
    opleidingsniveau: 'WO', werkervaring: '6 jaar',
    huidigeRol: 'Backend Developer bij CloudBase',
    voorkeursFunctiegebied1: 'ICT & Development', voorkeursFunctietitel1: 'Senior Software Developer',
    voorkeursFunctiegebied2: 'Data & Analytics', voorkeursFunctietitel2: 'Data Engineer',
    salarisMin: 5000, salarisMax: 6500, maxReistijd: '45 minuten', opKantoor: 'Hybride (2 dagen)',
    talen: [{ taal: 'Nederlands', niveau: 'C1' }, { taal: 'Engels', niveau: 'C1' }],
    scanCompleted: true, cvUploaded: true, scoutIds: ['scout-1'],
    poolStatus: 'in_proces',
  },
]

export const kandidaatSollicitaties: Sollicitatie[] = [
  { id: 's-1', vacatureTitle: 'Marketing Manager', company: 'TechVentures B.V.', status: 'Arbeidsvoorwaarden', datum: '2026-03-08', procesStatus: 'arbeidsvoorwaarden', vacatureId: 'vac-1' },
  { id: 's-2', vacatureTitle: 'Brand Strategist', company: 'CreativeCo', status: 'In behandeling', datum: '2026-03-12', procesStatus: 'gesprek_gepland', vacatureId: 'vac-ext-1' },
  { id: 's-3', vacatureTitle: 'Content Marketing Specialist', company: 'MediaHouse B.V.', status: 'Voorgesteld', datum: '2026-03-15', procesStatus: 'voorgesteld', vacatureId: 'vac-ext-2' },
  { id: 's-4', vacatureTitle: 'Growth Manager', company: 'ScaleUp Inc.', status: 'Afgewezen', datum: '2026-02-20', procesStatus: 'afgewezen', vacatureId: 'vac-ext-3' },
]

export function calculateFee(opleidingsniveau: string, werkervaring: string): { fee: number; scoutFee: number; refurzyFee: number } {
  const expPoints: Record<string, number> = { '0-2 jaar': 1, '2-5 jaar': 2, '5-10 jaar': 3, '>10 jaar': 4 }
  const eduPoints: Record<string, number> = { 'MBO': 1.5, 'HBO': 2, 'WO': 3 }
  const exp = expPoints[werkervaring] || 2
  const edu = eduPoints[opleidingsniveau] || 2
  const fee = exp * edu * 1200
  return { fee, scoutFee: fee * 0.5, refurzyFee: fee * 0.5 }
}

// ─── Mock contracts ─────────────────────────────────────────────────────────
export const mockContracten: Contract[] = [
  {
    id: 'contract-1',
    contractNummer: 'RF-2026-001',
    datum: '2026-02-15',
    opdrachtgeverNaam: 'Daan Verhoeven',
    opdrachtgeverBedrijf: 'TechVentures B.V.',
    opdrachtgeverKvk: '12345678',
    opdrachtgeverAdres: 'Herengracht 100, 1015 BS Amsterdam',
    vacatureId: 'vac-1',
    vacatureTitle: 'Marketing Manager',
    kandidaatId: 'k-1',
    kandidaatNaam: 'Anna de Jong',
    kandidaatInitialen: 'A',
    scoutNaam: 'Sophie de Graaf',
    opleidingsniveau: 'WO',
    werkervaring: '8 jaar',
    plaatsingsfee: 10800,
    btw: 2268,
    totaalInclBtw: 13068,
    scoutFee: 5400,
    fitGarantie: true,
    mScore: 87,
    status: 'actief',
    ondertekendDoor: 'Daan Verhoeven',
    ondertekendOp: '2026-02-15T14:32:00Z',
  },
  {
    id: 'contract-2',
    contractNummer: 'RF-2026-002',
    datum: '2026-03-05',
    opdrachtgeverNaam: 'Daan Verhoeven',
    opdrachtgeverBedrijf: 'TechVentures B.V.',
    opdrachtgeverKvk: '12345678',
    opdrachtgeverAdres: 'Herengracht 100, 1015 BS Amsterdam',
    vacatureId: 'vac-2',
    vacatureTitle: 'Senior Software Developer',
    kandidaatId: 'k-4',
    kandidaatNaam: 'Thomas van Dijk',
    kandidaatInitialen: 'T',
    scoutNaam: 'Sophie de Graaf',
    opleidingsniveau: 'WO',
    werkervaring: '10 jaar',
    plaatsingsfee: 14400,
    btw: 3024,
    totaalInclBtw: 17424,
    scoutFee: 7200,
    fitGarantie: true,
    mScore: 82,
    status: 'getekend',
    ondertekendDoor: 'Daan Verhoeven',
    ondertekendOp: '2026-03-05T10:15:00Z',
  },
  {
    id: 'contract-3',
    contractNummer: 'RF-2025-047',
    datum: '2025-06-20',
    opdrachtgeverNaam: 'Daan Verhoeven',
    opdrachtgeverBedrijf: 'TechVentures B.V.',
    opdrachtgeverKvk: '12345678',
    opdrachtgeverAdres: 'Herengracht 100, 1015 BS Amsterdam',
    vacatureId: 'vac-old-1',
    vacatureTitle: 'Product Owner',
    kandidaatId: 'k-old-1',
    kandidaatNaam: 'Sanne Visser',
    kandidaatInitialen: 'S',
    scoutNaam: 'Mark Jansen',
    opleidingsniveau: 'HBO',
    werkervaring: '5-10 jaar',
    plaatsingsfee: 7200,
    btw: 1512,
    totaalInclBtw: 8712,
    scoutFee: 3600,
    fitGarantie: false,
    mScore: 74,
    status: 'verlopen',
    ondertekendDoor: 'Daan Verhoeven',
    ondertekendOp: '2025-06-20T09:00:00Z',
  },
]

// ─── Pipeline step definitions ──────────────────────────────────────────────
export const pipelineSteps = [
  { key: 'voorgesteld', label: 'Voorgesteld', icon: '📋', color: 'text-ink-muted' },
  { key: 'contract_akkoord', label: 'Contract akkoord', icon: '✅', color: 'text-purple' },
  { key: 'gesprek_plannen', label: 'Gesprek plannen', icon: '📅', color: 'text-orange' },
  { key: 'gesprek_gepland', label: 'Gesprek gepland', icon: '🤝', color: 'text-blue-500' },
  { key: 'feedback_geven', label: 'Feedback geven', icon: '💬', color: 'text-yellow-600' },
  { key: 'vervolggesprek', label: 'Vervolggesprek', icon: '🔄', color: 'text-blue-500' },
  { key: 'arbeidsvoorwaarden', label: 'Arbeidsvoorwaarden', icon: '💼', color: 'text-purple' },
  { key: 'contract_getekend', label: 'Contract getekend', icon: '🎉', color: 'text-green-600' },
] as const

export const afwijzingsRedenen = [
  { key: 'ervaring', label: 'Niet de juiste ervaring' },
  { key: 'cultuur', label: 'Culturele mismatch' },
  { key: 'salaris', label: 'Salariseis te hoog' },
  { key: 'andere_kandidaat', label: 'Andere kandidaat gekozen' },
  { key: 'anders', label: 'Anders' },
] as const
