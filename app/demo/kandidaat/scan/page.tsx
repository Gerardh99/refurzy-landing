...
export default function ProfielMatchScan() {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold text-white">Profiel Match Scan</h1>
        <p className="text-gray-400 mt-1">
          Beantwoord 35 vragen over je werkinteresses, kernwaarden en ideale organisatiecultuur
        </p>
      </div>

      <div className="bg-purple/10 border border-purple/20 rounded-2xl p-5 flex items-start gap-3">
        <span className="text-purple-light text-lg mt-0.5">&#127891;</span>
        <div className="text-sm text-gray-300 space-y-1">
          <p className="font-medium text-purple-light">Wetenschappelijk onderbouwd</p>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          <span className="text-gray-400">{answeredCount} van {totalQuestions} vragen beantwoord</span>
      ...
      <div className="bg-navy-light rounded-2xl border border-purple/10 p-6 space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white">Werkinteresses</h2>
          <p className="text-gray-500 text-sm mt-1">19 items — hieronder een preview van 3 vragen</p>
        </div>
      ...
      <div className="bg-navy-light rounded-2xl border border-purple/10 p-6 space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white">Kernwaarden</h2>
          <p className="text-gray-500 text-sm mt-1">9 items — hieronder een preview van 2 vragen</p>
        </div>
      ...
      <div className="bg-navy-light rounded-2xl border border-purple/10 p-6 space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white">Organisatiecultuur</h2>
          <p className="text-gray-500 text-sm mt-1">7 types — hieronder een preview van 2 vragen</p>
        </div>
...
function LikertItem({ id, label, value, onChange }: { id: string; label: string; value?: number; onChange: (v: number) => void }) {
  ...
  return (
    <div className="space-y-3 border-b border-purple/5 pb-5 last:border-0 last:pb-0">
      <p className="text-gray-300 text-sm">{label}</p>
      <div className="flex items-center gap-2">
        <span className="text-xs text-gray-600 w-12 text-right shrink-0">1</span>
            ...
            <button
              ...
              className={`flex-1 h-10 rounded-lg text-sm font-medium transition-all ${
                value === n
                  ...
                  : 'bg-navy border border-purple/20 text-gray-400 hover:border-cyan/30 hover:text-gray-300'
              }`}
            >
              {n}
            </button>
        ...
        <span className="text-xs text-gray-600 w-12 shrink-0">5</span>
      </div>
      <div className="flex justify-between text-xs text-gray-600 px-14">
        <span>{labels[0]}</span>
        <span>{labels[4]}</span>
      </div>
    </div>
