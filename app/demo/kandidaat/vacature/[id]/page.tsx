...
const texts = {
  nl: {
    ...
    companyVisibleAtContract: 'Bedrijfsnaam wordt zichtbaar bij contract',
    vacancyDetails: 'Vacature details',
    salary: 'Salaris',
    contractType: 'Contracttype',
    office: 'Op kantoor',
    location: 'Locatie',
    maxCommute: 'Max reistijd',
    deadline: 'Deadline',
    experience: 'ervaring',
    aboutRole: 'Over de functie',
    whatWillYouDo: 'Wat ga je doen?',
    whatWeOffer: 'Wat bieden wij?',
    aboutTeam: 'Over het team',
    yourFitHardCriteria: 'Jouw fit op harde criteria',
    hardCriteriaLabel: 'Harde Criteria',
    mScoreAnalysis: 'M-Score analyse',
    totalMatchScore: 'Totale match score',
    mScoreUnavailable: 'M-Score nog niet beschikbaar',
    startMatchingScan: 'Start Matching Scan',
    basedOnScan: 'Gebaseerd op de Matching Scan (VU Amsterdam)',
    pipelineStatus: 'Pipeline status',
    currentStep: 'Huidige stap',
    completed: 'Afgerond',
    nominatedBy: 'Voorgedragen door',
    privacy: 'Privacy:',
    privacyDesc: 'Je profiel wordt anoniem gepresenteerd aan de opdrachtgever. Pas wanneer het contract wordt geaccepteerd, worden je contactgegevens gedeeld.',
    privacyAnon: ' De bedrijfsnaam wordt zichtbaar zodra de contractfase is bereikt.',
    // Criteria status labels
    meets: 'Voldoet',
    almost: 'Bijna',
    doesNotMeet: 'Voldoet niet',
    // Pipeline steps
    pipelineNominated: 'Voorgedragen',
    pipelineInterview: 'Gesprek',
    pipelineConditions: 'Arbeidsvoorwaarden',
    pipelineContract: 'Contract',
    // Bar chart dimensions
    dimWork: 'Werkzaamheden',
    dimValues: 'Waarden',
    dimOrg: 'Organisatie',
  },
  en: {
    ...
    companyVisibleAtContract: 'Company name will be visible at contract stage',
    vacancyDetails: 'Vacancy details',
    salary: 'Salary',
    contractType: 'Contract type',
    office: 'In office',
    location: 'Location',
    maxCommute: 'Max commute',
    deadline: 'Deadline',
    ...
    yourFitHardCriteria: 'Your fit on hard criteria',
    hardCriteriaLabel: 'Hard Criteria',
    mScoreAnalysis: 'M-Score analysis',
    totalMatchScore: 'Total match score',
    mScoreUnavailable: 'M-Score not yet available',
    startMatchingScan: 'Start Matching Scan',
    basedOnScan: 'Based on the Matching Scan (VU Amsterdam)',
    pipelineStatus: 'Pipeline status',
    currentStep: 'Current step',
    completed: 'Completed',
    nominatedBy: 'Nominated by',
    privacy: 'Privacy:',
    privacyDesc: 'Your profile is presented anonymously to the employer. Only when the contract is accepted will your contact details be shared.',
    privacyAnon: ' The company name will be visible once the contract stage is reached.',
    // Criteria status labels
    meets: 'Meets',
    almost: 'Almost',
    doesNotMeet: 'Does not meet',
    // Pipeline steps
    pipelineNominated: 'Nominated',
    pipelineInterview: 'Interview',
    pipelineConditions: 'Employment terms',
    pipelineContract: 'Contract',
    // Bar chart dimensions
    dimWork: 'Work activities',
    dimValues: 'Values',
    dimOrg: 'Organization',
  },
}
...
const pipelineStepsEn = [
  ...
  { key: 'arbeidsvoorwaarden', label: 'Employment terms', icon: '💼' },
  { key: 'contract', label: 'Contract', icon: '🎉' },
]
...
const vacatureDetails: Record<string, {
  ...
  'kv-1': {
    ...
    hardeCriteriaDetails: [
      ...
      { label: 'Salaris (€4.500 – €6.000)', status: 'voldoet' },
      { label: 'Nederlands (min. C1)', status: 'voldoet' },
      { label: 'Engels (min. B2)', status: 'voldoet' },
    ],
    pipelineStap: 2,
  ...
  'kv-2': {
    ...
    hardeCriteriaDetails: [
      ...
      { label: 'Salaris (€3.500 – €4.500)', status: 'voldoet' },
      { label: 'Nederlands (min. B2)', status: 'voldoet' },
      { label: 'Engels (min. B2)', status: 'voldoet' },
    ],
    pipelineStap: 1,
    scoutNaam: 'Sophie de Graaf',
  ...
  'kv-3': {
    ...
    hardeCriteriaDetails: [
      ...
      { label: 'Salaris (€5.000 – €7.000)', status: 'voldoet' },
      { label: 'Engels (min. C1)', status: 'voldoet' },
    ],
    pipelineStap: 0,
...
function BarChart({ dimensions, t }: { dimensions: { werkzaamheden: number; waarden: number; organisatie: number }; t: typeof texts['nl'] }) {
  const bars = [
    { label: t.dimWork, value: dimensions.werkzaamheden, color: 'bg-cyan' },
    { label: t.dimValues, value: dimensions.waarden, color: 'bg-purple' },
    { label: t.dimOrg, value: dimensions.organisatie, color: 'bg-cyan-light' },
  ]
...
function HardeCriteriaFitBadge({ score, label }: { score: number; label: string }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border ${bg}`}>
      ...
      <span className="text-xs text-ink-muted ml-1">{label}</span>
    </div>
  )
}
  ...
  if (!vac) {
    return (
      <div className="space-y-4">
        <Link href="/demo/kandidaat" className="text-cyan text-sm hover:underline">{t.backToOverview}</Link>
        <div className="bg-white rounded-2xl border border-surface-border p-8 text-center">
          <p className="text-ink-muted">{t.vacancyNotFound}</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-2xl border border-surface-border p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-ink-light font-medium mt-1">
              {vac.bedrijfAnoniem ? (
                <span className="text-ink-muted italic">{t.companyVisibleAtContract}</span>
              ) : (
          ...
          <div className="flex items-center gap-3">
            {vac.hardeCriteriaFit !== null && (
              <HardeCriteriaFitBadge score={vac.hardeCriteriaFit} label={t.hardCriteriaLabel} />
            )}
            {vac.mScore !== null && (
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <span className="text-xs bg-surface-muted text-ink-light px-3 py-1 rounded-full">{vac.werkervaring} {t.experience}</span>
      ...
      <div className="bg-white rounded-2xl border border-surface-border p-6">
        <h2 className="text-lg font-semibold text-ink mb-4">{t.vacancyDetails}</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div>
            <div className="text-xs text-ink-muted mb-1">{t.salary}</div>
            <div className="text-sm text-ink font-medium">{vac.salaris}</div>
          </div>
          <div>
            <div className="text-xs text-ink-muted mb-1">{t.contractType}</div>
            <div className="text-sm text-ink font-medium">{vac.contractType}</div>
          </div>
          <div>
            <div className="text-xs text-ink-muted mb-1">{t.office}</div>
            <div className="text-sm text-ink font-medium">{vac.opKantoor}</div>
          </div>
          <div>
            <div className="text-xs text-ink-muted mb-1">{t.location}</div>
            <div className="text-sm text-ink font-medium">{vac.locatie}, {vac.land}</div>
          </div>
          <div>
            <div className="text-xs text-ink-muted mb-1">{t.maxCommute}</div>
            <div className="text-sm text-ink font-medium">{vac.maxReistijd}</div>
          </div>
          <div>
            <div className="text-xs text-ink-muted mb-1">{t.deadline}</div>
            <div className="text-sm text-ink font-medium">{new Date(vac.deadline).toLocaleDateString(lang === 'en' ? 'en-GB' : 'nl-NL')}</div>
          </div>
        </div>
      </div>
      ...
      {/* Omschrijving */}
      <div className="bg-white rounded-2xl border border-surface-border p-6">
        <h2 className="text-lg font-semibold text-ink mb-3">{t.aboutRole}</h2>
        <p className="text-sm text-ink-light leading-relaxed">{vac.omschrijving}</p>
      </div>

      {/* Taken */}
      <div className="bg-white rounded-2xl border border-surface-border p-6">
        <h2 className="text-lg font-semibold text-ink mb-3">{t.whatWillYouDo}</h2>
      ...
      {/* Wat bieden wij */}
      <div className="bg-white rounded-2xl border border-surface-border p-6">
        <h2 className="text-lg font-semibold text-ink mb-3">{t.whatWeOffer}</h2>
        <ul className="space-y-2">
      ...
      {/* Afdelingscultuur */}
      <div className="bg-white rounded-2xl border border-surface-border p-6">
        <h2 className="text-lg font-semibold text-ink mb-3">{t.aboutTeam}</h2>
      ...
      {vac.hardeCriteriaDetails && (
        <div className="bg-white rounded-2xl border border-surface-border p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-ink">{t.yourFitHardCriteria}</h2>
            {vac.hardeCriteriaFit !== null && (
              <HardeCriteriaFitBadge score={vac.hardeCriteriaFit} label={t.hardCriteriaLabel} />
            )}
          </div>
            ...
            {vac.hardeCriteriaDetails.map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className={`text-xs font-medium ${
                  ...
                  {item.status === 'voldoet' ? t.meets : item.status === 'bijna' ? t.almost : t.doesNotMeet}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* M-Score */}
        <div className="bg-white rounded-2xl border border-surface-border p-6">
          <h2 className="text-lg font-semibold text-ink mb-4">{t.mScoreAnalysis}</h2>
          {vac.mScoreDimensions ? (
            <div>
              <div className="text-center mb-6">
                ...
                <p className="text-xs text-ink-muted mt-2">{t.totalMatchScore}</p>
              </div>
              <BarChart dimensions={vac.mScoreDimensions} t={t} />
            ...
            <div className="text-center py-6">
              ...
              <p className="text-ink-muted text-sm">{t.mScoreUnavailable}</p>
        ...
        {/* Pipeline */}
        <div className="bg-white rounded-2xl border border-surface-border p-6">
          <h2 className="text-lg font-semibold text-ink mb-4">{t.pipelineStatus}</h2>
          <div className="space-y-4">
              ...
              return (
                <div key={step.key} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    {isActive && (
                      <p className="text-[10px] text-purple/70 mt-0.5">{t.currentStep}</p>
                    )}
                  </div>
                  {isDone && (
          ...
          <div className="mt-6 pt-4 border-t border-surface-border">
            <p className="text-xs text-ink-muted">
              {t.nominatedBy} <span className="text-ink font-medium">{vac.scoutNaam}</span>
            </p>
            <p className="text-xs text-ink-muted mt-1">
              {new Date(vac.datum).toLocaleDateString(lang === 'en' ? 'en-GB' : 'nl-NL')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface-muted rounded-xl p-4 text-xs text-ink-light">
        <p>
          <strong className="text-ink">{t.privacy}</strong> {t.privacyDesc}
          {vac.bedrijfAnoniem && t.privacyAnon}
        </p>
      </div>
    </div>
  )
